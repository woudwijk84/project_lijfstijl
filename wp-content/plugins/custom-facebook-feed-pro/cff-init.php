...
function display_cff($atts, $content = null) {
    ...
    foreach ( $page_ids as $page_id ) {
        ...
        if (!$cff_events_only || ($cff_events_only && $cff_events_source == 'timeline') ){
            ...
            //VIDEOS ONLY
            if($cff_videos_only){
                $cff_posts_json_url = 'https://graph.facebook.com/'.$page_id.'?fields=videos{source,name,description,embed_html,format{picture}}&access_token='.$access_token.'&locale='.$cff_locale . $cff_date_range;
            }
            ...
            //ALBUMS ONLY
            if($cff_albums_only && $cff_albums_source == 'photospage') $cff_posts_json_url = 'https://graph.facebook.com/' . $page_id . '/albums?fields=id,name,description,link,cover_photo,count,created_time&access_token=' . $access_token . '&limit=' . $cff_post_limit. '&locale=' . $cff_locale . $cff_date_range;
